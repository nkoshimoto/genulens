                                ...
                                int *n_active_out)
{
    ...
    if (hist_sum <= 0 || total_draws <= 0) {
        *max_relerr_out = -1.0; *pass_frac_out = 0.0; *n_active_out = 0;
        return 0;
    }
    int n_active = 0, n_pass = 0;
    double max_relerr = 0.0, cdf = 0.0;
...
int main(int argc, char **argv)
{
    ...
    // ---- Initialize galaxy model with kinematics ----
    ...
    if (ctx.density.ND)
        ctx.density.rho0ND = (ctx.density.ND == 3) ? 1.0 : 0.25*MND/PI/ctx.density.x0ND/ctx.density.y0ND/ctx.density.z0ND;
    ...
    kinematic_tables.initialize_shu_distribution(ctx);
    ...
    for (int i = 0; i < 8; i++) tau_median[i] = ctx.kinematics.medtauds[i];
    tau_median[8]  = ctx.stellar.mageB;
    tau_median[9]  = ctx.stellar.mageND;
    tau_median[10] = 12.0;
    ...
    if (AUTOERR)
        printf("# AUTOERR=1: target_relerr=%.3f, check_every=%ld, CDF=[%.3f, %.3f], min_prob=%.2e, required_frac=%.3f\n"
               "# Nsimu is treated as the maximum accepted draw count.\n",
    ...
    if (GRID == 1) {
        ...
        for (int i_DL = 0; i_DL < nDL; i_DL++) {
            ...
            double xyz[3]={}, xyb[2]={};
            calc_rho_each(ctx, DL_c, 0, rhos_L, xyz, xyb);
            calc_rho_each(ctx, DS_c, 0, rhos_S, xyz, xyb);

            double wL[11]={}, wS[11]={}, wL_tot=0, wS_tot=0;
            for (int i = 0; i < ncomp; i++) {
                wL[i] = n0MS_arr[i] * rhos_L[i];
                wS[i] = n0MS_arr[i] * rhos_S[i];
                wL_tot += wL[i]; wS_tot += wS[i];
            }
            ...
            int converged_mu = 0, converged_phi = 0;

            while (wtot < Nsimu) {
                double ran_L = ctx.runtime.rng->uniform();
                int i_l = ncomp - 1;
                for (int i = 0; i < ncomp; i++) if (ran_L < cumwL[i]) { i_l = i; break; }

                double ran_S = ctx.runtime.rng->uniform();
                int i_s = ncomp - 1;
                for (int i = 0; i < ncomp; i++) if (ran_S < cumwS[i]) { i_s = i; break; }

                if ((i_l == 9 || i_s == 9) && ctx.density.ND == 0) continue;

                double vxyz_L[3]={}, vxyz_S[3]={};
                get_vxyz_ran(ctx, vxyz_L, i_l, tau_median[i_l], DL_c, lSIMU, bSIMU);
                get_vxyz_ran(ctx, vxyz_S, i_s, tau_median[i_s], DS_c, lSIMU, bSIMU);

                double muLl = (( vxyz_L[0]-ctx.kinematics.vxsun)*sinl + (vxyz_L[1]-ctx.kinematics.vysun)*cosl) * KS2MY / DL_c;
                double muLb = (( vxyz_L[0]-ctx.kinematics.vxsun)*cosl*sinb - (vxyz_L[1]-ctx.kinematics.vysun)*sinl*sinb + (vxyz_L[2]-ctx.kinematics.vzsun)*cosb) * KS2MY / DL_c;
                double muSl = (( vxyz_S[0]-ctx.kinematics.vxsun)*sinl + (vxyz_S[1]-ctx.kinematics.vysun)*cosl) * KS2MY / DS_c;
                double muSb = (( vxyz_S[0]-ctx.kinematics.vxsun)*cosl*sinb - (vxyz_S[1]-ctx.kinematics.vysun)*sinl*sinb + (vxyz_S[2]-ctx.kinematics.vzsun)*cosb) * KS2MY / DS_c;

                double mul   = muLl - muSl, mub = muLb - muSb;
            ...
            if (AUTOERR)
                printf("# precision DS=%.1f DL=%.1f Ndraw=%.0f "
                       ...
                       "converged=%d\n",
                       DS_c, DL_c, wtot, mu_overflow, mu_overflow/wtot,
                       mu_n_active, mu_max_relerr, mu_pass_frac, converged_mu,
                       phi_n_active, phi_max_relerr, phi_pass_frac, converged_phi, converged);
            ...
            for (int ib = 0; ib < nout; ib++) {
                double mu_c  = (ib < nbins)    ? (ib + 0.5) * dmu                  : 0.0;
                double mu_d  = (ib < nbins)    ? hist[ib]                           : 0.0;
                ...
                if (AUTOERR) {
                    ...
                    printf("\t%.0f\t%.6e\t%.0f\t%.6e\t%.0f",
                           mc, mc>0?1/sqrt(mc):0.0, pc, pc>0?1/sqrt(pc):0.0, wtot);
                }
                printf("\n");
            }
        }} // i_DL, j_DS
    ...
    // ====================================================================
        ...
        calc_rho_each(ctx, Dl, 0, rhos_L, xyz_L, xyb_L);
        calc_rho_each(ctx, Ds, 0, rhos_S, xyz_S, xyb_S);
        ...
        while (wtot < Nsimu) {
            ...
            double tau_l = tau_median[i_l], tau_s = tau_median[i_s];
            double vxyz_L[3]={}, vxyz_S[3]={};
            get_vxyz_ran(ctx, vxyz_L, i_l, tau_l, Dl, lSIMU, bSIMU);
            get_vxyz_ran(ctx, vxyz_S, i_s, tau_s, Ds, lSIMU, bSIMU);

            double muLl = ((vxyz_L[0]-ctx.kinematics.vxsun)*sinl + (vxyz_L[1]-ctx.kinematics.vysun)*cosl) * KS2MY / Dl;
            double muLb = ((vxyz_L[0]-ctx.kinematics.vxsun)*cosl*sinb - (vxyz_L[1]-ctx.kinematics.vysun)*sinl*sinb + (vxyz_L[2]-ctx.kinematics.vzsun)*cosb) * KS2MY / Dl;
            double muSl = ((vxyz_S[0]-ctx.kinematics.vxsun)*sinl + (vxyz_S[1]-ctx.kinematics.vysun)*cosl) * KS2MY / Ds;
            double muSb = ((vxyz_S[0]-ctx.kinematics.vxsun)*cosl*sinb - (vxyz_S[1]-ctx.kinematics.vysun)*sinl*sinb + (vxyz_S[2]-ctx.kinematics.vzsun)*cosb) * KS2MY / Ds;
        ...
        if (AUTOERR) {
            ...
            printf("# precision Ndraw=%.0f "
                   ...
                   "phi_active_bins=%d phi_max_relerr=%.4f phi_pass_frac=%.3f phi_converged=%d "
                   "converged=%d\n",
                   wtot, mu_overflow, mu_overflow/wtot,
                   mu_n_active, mu_max_relerr, mu_pass_frac, converged_mu,
                   phi_n_active, phi_max_relerr, phi_pass_frac, converged_phi, converged);
        }
        ...
        for (int ib = 0; ib < nout; ib++) {
            ...
            double phi_c = (ib < nphibins) ? phimin + (ib + 0.5) * dphi : 0.0;
            double phi_d = (ib < nphibins) ? phi_hist[ib]                : 0.0;
            printf("%.3f\t%.6f\t%.6e\t%.6e", mu_c, phi_c, mu_d, phi_d);
            if (VERBOSE >= 1)
                for (int i = 0; i < ncomp; i++)
                    printf("\t%.6e", (ib < nbins) ? hist_comp[i][ib] : 0.0);
            if (AUTOERR) {
                double mc = (ib < nbins)    ? hist[ib]*wtot*dmu     : 0.0;
                double pc = (ib < nphibins) ? phi_hist[ib]*wtot*dphi: 0.0;
                printf("\t%.0f\t%.6e\t%.0f\t%.6e\t%.0f",
...
cleanup:
    kinematic_tables.release_all(ctx);
